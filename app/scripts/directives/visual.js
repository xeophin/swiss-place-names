(function ($, d3, angular) {
  ...
  function visual(suffixes, placeNames) {
    ...
    function link(scope, element, attrs, vm) {
      ...
      // Setup attributes
      vm.title = attrs.displayTitle || false;
      vm.chosenSuffix = attrs.searchString || '';
      vm.showSelfService = attrs.showSelfService || false;
      vm.showLegend = attrs.showLegend || false;
      vm.subhead = attrs.subhead || false;
      vm.showPopup = attrs.showPopup || false;
       ...
       * @type {string}
       */
      vm.identifier = '.' + attrs.class;

      vm.hexbinSize = attrs.hexbinSize || 10;
  ...
  function VisualController (){
    ...
    // Public properties
    var vm = this;
    vm.drawBackgroundGrid = drawBackgroundGrid;
    vm.drawCities = drawCities;
    vm.drawLegend = drawLegend;
    vm.drawPlaceNames = drawPlaceNames;
    ...
    function drawLegend() {
      ...
      d3.select(vm.identifier + ' .legend')
        .selectAll('.legendbin')
        .data(legendData)
        .enter()
        .append('div')
    ...
    function drawPlaceNames(svg, regex) {
      ...
      var points = vm.allPlacenames.filter(function (value, index) {
        return reg.test(value.namewithoutadditions);
      });
      vm.foundPlaces = points.length;
      var bins = hexbin(points);

      // Delete previous hexbins
      d3.selectAll(vm.identifier + ' .hexagon').remove();
    ...
    function setupSvg() {
      // Setup SVG
      var svg = d3.select(vm.identifier + ' .d3.graph').append('svg')
        .attr('width', '100%')
        .attr('height', '100%')
